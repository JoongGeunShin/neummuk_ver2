...
class ExploreOverlay extends ConsumerStatefulWidget {
  ...
  @override
  ConsumerState<ExploreOverlay> createState() => _ExploreOverlayState();
}

class _ExploreOverlayState extends ConsumerState<ExploreOverlay> {
  ...
  void initState() {
    super.initState();
    widget.events.onMapTapped = (_, __) {
  ...
  @override
  void didUpdateWidget(covariant ExploreOverlay old) {
  ...
  Future<void> _initLocation() async {
    ...
    final pos = await fetchMapPosition(
        accuracy: LocationAccuracy.high, timeout: const Duration(seconds: 10));
    if (!mounted) return;
    if (pos != null) {
      lat = pos.latitude;
      lng = pos.longitude;
  ...
  Future<void> _goToCurrentLocation() async {
    if (_locating || _ctrl == null) return;
  ...
  Future<int> _getVisibleRadiusMeters() async {
    ...
    try {
      final results = await Future.wait([
        _ctrl!.getCameraPosition(),
        _ctrl!.getContentBounds(),
      ]);
