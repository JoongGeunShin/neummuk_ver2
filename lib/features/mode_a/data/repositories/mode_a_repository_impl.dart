...
import '../services/kakao_food_place_search_service.dart';
...
class ModeARepositoryImpl implements ModeARepository {
  ...
  Future<RouteResultEntity> getRoute({
    ...
    if (originLat == null ||
        ...
        destLat == null ||
        destLng == null) {
      throw Exception('출발지 또는 목적지 좌표를 확인할 수 없어요.');
    }

    if (transport == 'transit') {
      return _getODsayRoute(
        from: from,
        to: to,
        originLat: originLat,
        originLng: originLng,
  ...
  Future<RouteResultEntity> _getTmapWalkBikeRoute({
    ...
    if (result.source == null) {
      throw Exception('경로를 찾을 수 없어요. 잠시 후 다시 시도해주세요.');
    }
  ...
  // 시간표 데이터가 아니므로 자동차 기준 경로를 근사치로 보여준다.
  Future<RouteResultEntity> _getKakaoFallbackForTransit({
    ...
    final routePoints = kakao.points
        .map((p) => LatLng(latitude: p.lat, longitude: p.lng))
        .toList();
    final distanceKm = double.parse(
      (kakao.distanceM / 1000.0).toStringAsFixed(1),
    );
    final durationSec = kakao.durationSec > 0
        ? kakao.durationSec
        : max(60, kakao.distanceM ~/ 8);
  ...
  Future<RouteResultEntity> _getODsayRoute({
    ...
    if (response.statusCode != 200) {
      ...
      return _getKakaoFallbackForTransit(
        from: from,
        to: to,
        originLat: originLat,
        originLng: originLng,
    ...
    final distanceKm = double.parse(
      (totalDistanceM / 1000.0).toStringAsFixed(1),
    );

    final subPaths = path['subPath'] as List? ?? [];

    // ── TMAP 도보 경로 pre-fetch: 첫 번째/마지막 도보 구간만 (최대 2회 호출) ──
    ({int idx, double sLat, double sLng, double eLat, double eLng})? firstWalk,
        lastWalk;
    ...
    for (int i = 0; i < subPaths.length; i++) {
      ...
      double sLat = i == 0
          ? originLat
          : (double.tryParse(sub['startY']?.toString() ?? '') ?? 0.0);
      double sLng = i == 0
          ? originLng
          : (double.tryParse(sub['startX']?.toString() ?? '') ?? 0.0);
      double eLat = i == subPaths.length - 1
          ? destLat
          : (double.tryParse(sub['endY']?.toString() ?? '') ?? 0.0);
      double eLng = i == subPaths.length - 1
          ? destLng
          : (double.tryParse(sub['endX']?.toString() ?? '') ?? 0.0);
    ...
    if (mapObj != null && mapObj.isNotEmpty) {
      fetchFutures.add(() async {
        ...
        for (int i = 0; i < subPaths.length; i++) {
          final tt =
              ((subPaths[i] as Map<String, dynamic>)['trafficType'] as num? ??
                      3)
                  .toInt();
          if (tt != 3) {
    ...
    for (int i = 0; i < subPaths.length; i++) {
      ...
      if (trafficType == 3) {
        ...
        if (tmapPts.isNotEmpty) {
          ...
          if (i == subPaths.length - 1) {
            ...
            if (ex != 0 && ey != 0) {
              ...
              final r = _walkEndFromNextSeg(subPaths, i);
              if (r != null)
                segPts.add(LatLng(latitude: r.lat, longitude: r.lng));
            }
          }
        }
      } else {
        ...
        final hasClipBounds = bLat != 0 && bLng != 0 && aLat != 0 && aLng != 0;
        ...
        debugPrint(
          '[Transit seg$i] type=$trafficType pts=${segPts.length} '
          '(lane=${lanePts.length})',
        );
      }
  ...
  static ({double lat, double lng})? _walkStartFromPrevSeg(
    List subPaths,
    int walkIdx,
  ) {
    ...
    final stops =
        (prev['passStopList'] as Map<String, dynamic>?)?['stations'] as List?;
  ...
  static ({double lat, double lng})? _walkEndFromNextSeg(
    List subPaths,
    int walkIdx,
  ) {
    ...
    final stops =
        (next['passStopList'] as Map<String, dynamic>?)?['stations'] as List?;
    if (stops != null && stops.isNotEmpty) {
    ...
    try {
      ...
      for (final lane in laneList) {
        ...
        for (final section in sections) {
          final graphPos =
              (section as Map<String, dynamic>)['graphPos'] as List? ?? [];
    ...
    for (int i = 0; i < subPaths.length; i++) {
      ...
      if (trafficType == 2) {
        ...
        if (lanes.isNotEmpty) {
          final busNos = lanes
              .map(
                (l) => (l as Map<String, dynamic>)['busNo']?.toString() ?? '',
              )
  ...
  Future<List<RestaurantEntity>> getNearbyRestaurants({
    ...
    try {
      final matchedFoods = await _foodCatalogRepo.getFoodsNearKcal(
        targetKcal.toDouble(),
        AppConstants.kcalMatchTolerancePct,
      );
      ...
      final results = await Future.wait(
        candidates.map(
          (food) => _fetchKakaoFoodPlacesByKeyword(
            food,
            latitude,
            longitude,
            radiusM,
  ...
  /// [food]의 이름을 키워드로 Kakao Local(FD6)에서 실제 매장을 찾는다.
  /// mode_match(오늘의 맞춤 맛집)와 공유하는 KakaoFoodPlaceSearchService에 위임.
  Future<List<RestaurantEntity>> _fetchKakaoFoodPlacesByKeyword(
    ...
    return KakaoFoodPlaceSearchService.byKeyword(
      food: food,
      lat: lat,
      lng: lng,
      radiusM: radiusM,
  ...
  Future<List<WaypointCandidateEntity>> getWaypointCandidates({
    ...
    try {
      ...
      final candidates = results
          .map<WaypointCandidateEntity?>((item) {
            ...
            final detourKm = double.parse(
              (detourM / 1000.0).toStringAsFixed(1),
            );
            ...
            final category = contentTypeId == 28 ? '레포츠' : '관광지';
          ...
          .whereType<WaypointCandidateEntity>()
          .toList();

      // extraKcal이 목표에 가까운 순으로 정렬, 상위 5개 반환
      candidates.sort(
        (a, b) => (a.extraKcal - extraKcalNeeded).abs().compareTo(
  ...
  Future<List<PlaceEntity>> getNearbyPlaces({
    ...
    final places = await _fetchNearbyPlacesAtRadius(
      latitude,
      longitude,
      radiusKm,
      contentTypeId,
  ...
  static String? _buildMapObj(List subPaths) {
    ...
    for (final sub in subPaths) {
      ...
      final stations =
          (subMap['passStopList'] as Map<String, dynamic>?)?['stations']
              as List? ??
          [];
      final startIdx = stations.isNotEmpty
          ? (stations.first as Map<String, dynamic>)['index']?.toString() ??
                '-1'
          : '-1';
      ...
      if (tt == 2) {
        // 버스: class=1, busID 사용
        final busId =
            lane['busID']?.toString() ?? lane['busLocalBlID']?.toString();
        if (busId != null && busId.isNotEmpty)
          parts.add('$busId:1:$startIdx:$endIdx');
      } else {
        // 지하철: class=2, subwayCode 사용
        final code = lane['subwayCode']?.toString();
        if (code != null && code.isNotEmpty)
          parts.add('$code:2:$startIdx:$endIdx');
      }
    }
  ...
  static List<LatLng> _clipPolylineToSegment(
    ...
    int startIdx = 0;
    double startDist = double.infinity;
    int endIdx = 0;
    double endDist = double.infinity;
